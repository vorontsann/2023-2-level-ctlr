...
def warn(msg, error_type, testlevel=0, testid='some-test', lineno=True, nodelineno=0, nodeid=0):
    ...
    if not args.quiet:
        if args.max_err > 0 and error_counter[error_type] == args.max_err:
            ...
            if nodelineno:
                print("[%sLine %d%s%s]: [L%d %s %s] %s" % (
                fn, nodelineno, sent, node, testlevel, error_type, testid, msg), file=sys.stderr)
            elif lineno:
                print(
                    "[%sLine %d%s%s]: [L%d %s %s] %s" % (fn, curr_line, sent, node, testlevel, error_type, testid, msg),
                    file=sys.stderr)
            else:
                print("[%sTree number %d on line %d%s%s]: [L%d %s %s] %s" % (
                fn, tree_counter, sentence_line, sent, node, testlevel, error_type, testid, msg), file=sys.stderr)
    ...
    for line_counter, line in enumerate(inp):
        ...
        if is_whitespace(line):
            ...
            if len(cols) != COLCOUNT:
                ...
                testmessage = 'The line has %d columns but %d are expected. The contents of the columns will not be checked.' % (
                len(cols), COLCOUNT)
                warn(testmessage, testclass, testlevel=testlevel, testid=testid)
                corrupted = True
            ...
            testid = 'invalid-line'
            testmessage = "Spurious line: '%s'. All non-empty lines should start with a digit or the # character." % (
                line)
            warn(testmessage, testclass, testlevel=testlevel, testid=testid)
    else:  # end of file
...
def validate_unicode_normalization(text):
    ...
    if text != normalized_text:
        ...
        COLNAMES[firsti], firstj, inpfirst, nfcfirst)
        warn(testmessage, testclass, testlevel=testlevel, testid=testid)
    ...
    for col_idx in range(MISC + 1):
        ...
        if not cols[col_idx]:
            ...
            if whitespace2_re.match(cols[col_idx]):
                ...
                testmessage = 'Two or more consecutive whitespace characters not allowed in column %s.' % (
                COLNAMES[col_idx])
                warn(testmessage, testclass, testlevel=testlevel, testid=testid)
    ...
    if is_multiword_token(cols):
        for col_idx in (FORM, LEMMA):
            ...
            if whitespace_re.match(cols[col_idx]):
                ...
                testmessage = "White space not allowed in multi-word token '%s'. If it contains a space, it is not one surface token." % (
                cols[col_idx])
                warn(testmessage, testclass, testlevel=testlevel, testid=testid)
        ...
        for cols in tree:
            ...
            if not stext.startswith(cols[FORM]):
                if not mismatch_reported:
                    ...
                    testmessage = "Mismatch between the text attribute and the FORM field. Form[%s] is '%s' but text is '%s...'" % (
                ...
                if 'SpaceAfter=No' in cols[MISC].split("|"):
                    ...
                    if args.check_space_after and (stext) and not stext[0].isspace():
                        ...
                        testmessage = "'SpaceAfter=No' is missing in the MISC field of node #%s because the text is '%s'." % (
                        cols[ID], shorten(cols[FORM] + stext))
                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                             nodelineno=sentence_line + iline)
                    stext = stext.lstrip()
        if stext:
...
def validate_cols(cols, tag_sets, args):
    ...
    if is_word(cols) or is_empty_node(cols):
        ...
        validate_features(cols, tag_sets,
                          args)  # level 2 and up (relevant code checks whether higher level is required)
    elif is_multiword_token(cols):
...
attr_val_re = re.compile('^([A-Z][A-Za-z0-9]*(?:\[[a-z0-9]+\])?)=(([A-Z0-9][A-Z0-9a-z]*)(,([A-Z0-9][A-Z0-9a-z]*))*)$',
                         re.U)
        ...
        if match is None:
            ...
            for v in values:
                if not val_re.match(v):
                    ...
                    testmessage = "Spurious value '%s' in '%s'. Must start with [A-Z0-9] and only contain [A-Za-z0-9]." % (
                    v, f)
                    warn(testmessage, testclass, testlevel=testlevel, testid=testid)
                    ...
                    if effective_featset is not None:
                        if attr not in effective_featset:
                            ...
                            if lfrecord['permitted'] == 0:
                                ...
                                values = lfrecord['uvalues'] + lfrecord['lvalues'] + lfrecord['unused_uvalues'] + \
                                         lfrecord['unused_lvalues']
                                if not v in values:
                                    testid = 'feature-value-unknown'
                                    testmessage = "Value %s is not documented for feature %s in language [%s]." % (
                                    ...
                                    testmessage = "Feature %s is not permitted with UPOS %s in language [%s]." % (
                                    ...
                                    testmessage = "Value %s of feature %s is not permitted with UPOS %s in language [%s]." % (
    ...
    if not line_of_first_morpho_feature:
        ...
        for testid in delayed_feature_errors:
            for occurrence in delayed_feature_errors[testid]['occurrences']:
                warn(delayed_feature_errors[testid]['message'], delayed_feature_errors[testid]['class'],
                     testlevel=delayed_feature_errors[testid]['level'], testid=testid, nodeid=occurrence['nodeid'],
                     nodelineno=occurrence['nodelineno'])
    ...
    if not fv in feats.split('|'):
        if line_of_first_morpho_feature:
            ...
            if not testid in delayed_feature_errors:
                delayed_feature_errors[testid] = {'class': testclass, 'level': testlevel, 'message': testmessage,
                                                  'occurrences': []}
...
def validate_upos_vs_deprel(id, tree):
    ...
    if deprel == 'advmod' and not re.match(r"^(ADV|ADJ|CCONJ|DET|PART|SYM)",
                                           cols[UPOS]) and not 'fixed' in childrels and not 'goeswith' in childrels:
    ...
    if deprel == 'mark' and re.match(r"^(NOUN|PROPN|ADJ|PRON|DET|NUM|AUX|INTJ)",
                                     cols[UPOS]) and not 'fixed' in childrels:
        testid = 'rel-upos-mark'
        testmessage = "'mark' should not be '%s'" % (cols[UPOS])
        warn(testmessage, testclass, testlevel=testlevel, testid=testid, nodeid=id, nodelineno=tree['linenos'][id])
    # Cc is a conjunction, possibly an adverb or particle.
    if deprel == 'cc' and re.match(r"^(NOUN|PROPN|ADJ|PRON|DET|NUM|VERB|AUX|INTJ)",
                                   cols[UPOS]) and not 'fixed' in childrels:
        testid = 'rel-upos-cc'
...
def validate_single_subject(id, tree):
    ...
    if len(subjects) > 1:
        ...
        testmessage = "Node has multiple subjects not subtyped as ':outer': %s. Outer subjects are allowed if a clause acts as the predicate of another clause." % str(
            subjects)
        warn(testmessage, testclass, testlevel=testlevel, testid=testid, nodeid=id, nodelineno=tree['linenos'][id])
...
def validate_functional_leaves(id, tree):
    ...
    if re.match(r"^(case|mark|cc|aux|cop|det|fixed|goeswith|punct)$", deprel):
        ...
        for idchild in tree['children'][id]:
            ...
            if pdeprel != 'punct' and cdeprel == 'advmod' and re.match(r"^(PART|ADV)$",
                                                                       cupos) and 'Polarity=Neg' in cfeats:
                continue
            ...
            if re.match(r"^(mark|case)$", pdeprel) and not re.match(
                ...
                pdeprel, idparent, tree['nodes'][idparent][FORM], pdeprel, idchild, tree['nodes'][idchild][FORM],
                cdeprel)
                warn(testmessage, testclass, testlevel=testlevel, testid=testid, nodeid=id,
            ...
            ###!!! of errors.
            if re.match(r"^(aux|cop)$", pdeprel) and not re.match(r"^(goeswith|fixed|reparandum|conj|cc|punct)$",
                                                                  cdeprel):
                testid = 'leaf-aux-cop'
                testmessage = "'%s' not expected to have children (%s:%s:%s --> %s:%s:%s)" % (
                pdeprel, idparent, tree['nodes'][idparent][FORM], pdeprel, idchild, tree['nodes'][idchild][FORM],
...
def validate_goeswith_span(id, tree):
    ...
    if gwchildren:
        ...
        validate_required_feature(tree['nodes'][id][FEATS], 'Typo=Yes', testmessage, testlevel, testid, id,
                                  tree['linenos'][id])
...
def validate_goeswith_morphology_and_edeps(id, tree):
    ...
    if lspec2ud(tree['nodes'][id][DEPREL]) == 'goeswith':
        ...
        if tree['nodes'][id][FEATS] != '_':
            ...
            warn(testmessage, testclass, testlevel=testlevel, testid=testid, nodeid=id, nodelineno=tree['linenos'][id])
        testclass = 'Enhanced'
        if tree['nodes'][id][DEPS] != '_' and tree['nodes'][id][DEPS] != tree['nodes'][id][HEAD] + ':' + \
                tree['nodes'][id][DEPREL]:
    ...
    for col_idx in (FORM, LEMMA):
        ...
        if whitespace_re.match(cols[col_idx]) is not None:
            ...
            for regex in tag_sets[TOKENSWSPACE]:
                ...
                testmessage = "'%s' in column %s is not on the list of exceptions allowed to contain whitespace (data/tokens_w_space.LANG files)." % (
...
def validate_auxiliary_verbs(cols, children, nodes, line, lang, auxlist):
    ...
    if cols[UPOS] == 'AUX' and cols[LEMMA] != '_':
        ...
        if not lspecauxs:
            ...
            testclass = 'Morpho'
            testid = 'aux-lemma'
            testmessage = "'%s' is not an auxiliary verb in language [%s] (there are no known approved auxiliaries in this language)" % (
            cols[LEMMA], lang)
...
def validate_copula_lemmas(cols, children, nodes, line, lang, coplist):
    ...
    if cols[DEPREL] == 'cop' and cols[LEMMA] != '_':
        ...
        if not lspeccops:
            ...
            testclass = 'Syntax'
            testid = 'cop-lemma'
            testmessage = "'%s' is not a copula in language [%s] (there are no known approved copulas in this language)" % (
            cols[LEMMA], lang)
            warn(testmessage, testclass, testlevel=testlevel, testid=testid, nodeid=cols[ID], nodelineno=line)
...
def validate_misc_entity(comments, sentence):
        ...
        if global_entity_match:
            ...
            if line_of_global_entity:
                if global_entity_match.group(1) != global_entity_attribute_string:
                    ...
                    testmessage = "New declaration of global.Entity '%s' does not match the first declaration '%s' on line %d." % (
                    global_entity_match.group(1), global_entity_attribute_string, line_of_global_entity)
                    warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                         nodelineno=comment_start_line + iline)
            else:
                ...
                if not re.match(r'^[a-z]+(-[a-z]+)*$', global_entity_attribute_string):
                    ...
                    if not 'eid' in global_entity_attributes:
                        testid = 'spurious-global-entity'
                        testmessage = "Global.Entity attribute declaration '%s' does not include 'eid'." % (
                            global_entity_attribute_string)
                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                             nodelineno=comment_start_line + iline)
                    elif global_entity_attributes[0] != 'eid':
                        testid = 'spurious-global-entity'
                        testmessage = "Attribute 'eid' must come first in global.Entity attribute declaration '%s'." % (
                            global_entity_attribute_string)
                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                             nodelineno=comment_start_line + iline)
                    if not 'etype' in global_entity_attributes:
                        testid = 'spurious-global-entity'
                        testmessage = "Global.Entity attribute declaration '%s' does not include 'etype'." % (
                            global_entity_attribute_string)
                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                             nodelineno=comment_start_line + iline)
                    elif global_entity_attributes[1] != 'etype':
                        testid = 'spurious-global-entity'
                        testmessage = "Attribute 'etype' must come second in global.Entity attribute declaration '%s'." % (
                            global_entity_attribute_string)
                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                             nodelineno=comment_start_line + iline)
                    if not 'head' in global_entity_attributes:
                        testid = 'spurious-global-entity'
                        testmessage = "Global.Entity attribute declaration '%s' does not include 'head'." % (
                            global_entity_attribute_string)
                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                             nodelineno=comment_start_line + iline)
                    elif global_entity_attributes[2] != 'head':
                        testid = 'spurious-global-entity'
                        testmessage = "Attribute 'head' must come third in global.Entity attribute declaration '%s'." % (
                            global_entity_attribute_string)
                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                             nodelineno=comment_start_line + iline)
                    if 'other' in global_entity_attributes and global_entity_attributes[3] != 'other':
                        testid = 'spurious-global-entity'
                        testmessage = "Attribute 'other', if present, must come fourth in global.Entity attribute declaration '%s'." % (
                            global_entity_attribute_string)
                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                             nodelineno=comment_start_line + iline)
                    ...
                    for a in global_entity_attributes:
                        if a in entity_attribute_index:
                            testid = 'spurious-global-entity'
                            testmessage = "Attribute '%s' occurs more than once in global.Entity attribute declaration '%s'." % (
    ...
    for cols in sentence:
        ...
        if len(bridge) > 0 and len(entity) == 0:
            ...
            testmessage = "The 'Bridge=' statement can only occur together with 'Entity=' in MISC but we have %s." % (
                str(misc))
            warn(testmessage, testclass, testlevel=testlevel, testid=testid, nodelineno=sentence_line + iline)
            continue
        if len(splitante) > 0 and len(entity) == 0:
            testid = 'splitante-without-entity'
            testmessage = "The 'SplitAnte=' statement can only occur together with 'Entity=' in MISC but we have %s." % (
                str(misc))
            warn(testmessage, testclass, testlevel=testlevel, testid=testid, nodelineno=sentence_line + iline)
            continue
        ...
        if len(entity) > 0:
                ...
                for b, e in entities:
                    ...
                    if b == 0 or b == 2:
                        ...
                        if len(attributes) > entity_attribute_number:
                            ...
                            testmessage = "Entity '%s' has %d attributes while only %d attributes are globally declared." % (
                        ...
                        if len(attributes) > 1:
                            testid = 'too-many-entity-attributes'
                            testmessage = "Entity '%s' has %d attributes while only eid is expected at the closing bracket." % (
                            e, len(attributes))
                            warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                 nodelineno=sentence_line + iline)
                        beid = attributes[0]
                    ...
                    if match:
                        ...
                        if ipart == 1 and npart == 1:
                            ...
                            testmessage = "Discontinuous mention must have at least two parts but it has one in '%s'." % (
                                beid)
                            warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                 nodelineno=sentence_line + iline)
                        if ipart > npart:
                            testid = 'spurious-entity-id'
                            testmessage = "Entity id '%s' of discontinuous mention says the current part is higher than total number of parts." % (
                                beid)
                            warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                 nodelineno=sentence_line + iline)
                    else:
                        if re.match(r'[\[\]]', beid):
                            testid = 'spurious-entity-id'
                            testmessage = "Entity id '%s' contains square brackets but does not have the form used in discontinuous mentions." % (
                                beid)
                            warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                 nodelineno=sentence_line + iline)
                    head = 0
                    ...
                    def opening_bracket():
                        ...
                        if npart > 1:
                            ...
                            attributes_without_eid = [attributes[i] for i in range(len(attributes)) if
                                                      i != entity_attribute_index['eid']]
                            ...
                            if ipart == 1:
                                ...
                                discontinuous_mention = {'last_ipart': 1, 'npart': npart,
                                                         ...
                                                         'attributes': attrstring_to_match, 'length': 0, 'span': []}
                                ...
                                if eidnpart in open_discontinuous_mentions:
                                    ...
                                    if ipart != discontinuous_mention['last_ipart'] + 1:
                                        ...
                                        testmessage = "Unexpected part of discontinuous mention '%s': last part was '%d/%d' on line %d." % (
                                        beid, discontinuous_mention['last_ipart'], discontinuous_mention['npart'],
                                        ...
                                        testmessage = "Attribute mismatch of discontinuous mention: current part has '%s', first part '%s' was at line %d." % (
                                        attrstring_to_match, discontinuous_mention['attributes'],
                                        discontinuous_mention['first_part_line'])
                                        warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                             nodelineno=sentence_line + iline)
                                else:
                                    testid = 'misplaced-mention-part'
                                    testmessage = "Unexpected part of discontinuous mention '%s': this is part %d but we do not have information about the previous parts." % (
                                    beid, ipart)
                                    warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                         nodelineno=sentence_line + iline)
                        ...
                        if eid in entity_ids_other_documents:
                            ...
                            testmessage = "Same entity id should not occur in multiple documents; '%s' first seen on line %d, before the last newdoc." % (
                        ...
                        if 'etype' in entity_attribute_index and len(attributes) >= entity_attribute_index['etype'] + 1:
                            ...
                            if not re.match(
                                ...
                                warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                     nodelineno=sentence_line + iline)
                        if 'identity' in entity_attribute_index and len(attributes) >= entity_attribute_index[
                            'identity'] + 1:
                        ...
                        if 'head' in entity_attribute_index and len(attributes) >= entity_attribute_index['head'] + 1:
                            if not re.match(r'^[1-9][0-9]*$', attributes[entity_attribute_index['head']]):
                                ...
                                testmessage = "Entity head index '%s' must be a non-zero-starting integer." % (
                        ...
                        if not eid in entity_types:
                            ...
                            if etype != entity_types[eid][0]:
                                ...
                                testmessage = "Entity '%s' cannot have type '%s' that does not match '%s' from the first mention on line %d." % (
                            ...
                            if identity != entity_types[eid][1]:
                                ...
                                testmessage = "Entity '%s' cannot have identity '%s' that does not match '%s' from the first mention on line %d." % (
                                eid, identity, entity_types[eid][1], entity_types[eid][2])
                                warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                     nodelineno=sentence_line + iline)
                        # Remember the line where (the current part of) the entity mention starts.
                        mention = {'beid': beid, 'line': sentence_line + iline, 'span': [cols[ID]], 'text': cols[FORM],
                    ...
                    def closing_bracket():
                        ...
                        if len(open_entity_mentions) == 0:
                            ...
                            if beid != open_entity_mentions[-1]['beid']:
                                ...
                                testmessage = "Entity mentions are not well nested: closing '%s' while the innermost open entity is '%s' from line %d: %s." % (
                                beid, open_entity_mentions[-1]['beid'], open_entity_mentions[-1]['line'],
                                str(open_entity_mentions))
                                warn(testmessage, 'Warning', testlevel=testlevel, testid=testid,
                                     nodelineno=sentence_line + iline)
                            ...
                            for i in reversed(range(len(open_entity_mentions))):
                                ...
                                testid = 'ill-nested-entities'
                                testmessage = "Cannot close entity '%s' because it was not found among open entities: %s" % (
                        ...
                        if npart > 1:
                            ...
                            if eidnpart in open_discontinuous_mentions:
                                ...
                                warn(testmessage, 'Internal', testlevel=0, testid=testid,
                                     nodelineno=sentence_line + iline)
                                discontinuous_mention = {'last_ipart': ipart, 'npart': npart,
                                                         'first_part_line': opening_line,
                                                         'last_part_line': opening_line, 'attributes': '',
                                                         'length': mention_length, 'span': mention_span}
                                open_discontinuous_mentions[eidnpart] = [discontinuous_mention]
                        ...
                        if ipart == npart:
                            if mention_length < head:
                                ...
                                testmessage = "Entity mention head was specified as %d on line %d but the mention has only %d nodes." % (
                            ...
                            if ending_mention_key in ending_mentions:
                                ...
                                testmessage = "Entity mentions '%s' and '%s' from line %d have the same span %s." % (
                                ending_mentions[ending_mention_key], beid, opening_line, str(mention_span))
                                warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                     nodelineno=sentence_line + iline)
                            else:
                                ...
                                if sentid in entity_mention_spans[eid]:
                                    for m in entity_mention_spans[eid][sentid]:
                                        ...
                                        if ms.intersection(myset) and not ms.issubset(myset) and not myset.issubset(ms):
                                            ...
                                            testmessage = "Mentions of entity '%s' have crossing spans: %s vs. %s." % (
                    ...
                    if b == 0:
                        if seen2 and not seen1:
                            testid = 'spurious-entity-statement'
                            testmessage = "If there are no closing entity brackets, single-node entity must follow all opening entity brackets in '%s'." % (
                            entity[0])
                            warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                 nodelineno=sentence_line + iline)
                        if seen0 and seen2:
                            testid = 'spurious-entity-statement'
                            testmessage = "Single-node entity must either precede all closing entity brackets or follow all opening entity brackets in '%s'." % (
                            entity[0])
                            warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                 nodelineno=sentence_line + iline)
                        seen0 = True
                        ...
                        if seen1 and not seen0:
                            testid = 'spurious-entity-statement'
                            testmessage = "If there are no opening entity brackets, single-node entity must precede all closing entity brackets in '%s'." % (
                            entity[0])
                            warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                 nodelineno=sentence_line + iline)
                        seen2 = True
                        ...
                        if seen0:
                            testid = 'spurious-entity-statement'
                            testmessage = "All closing entity brackets must precede all opening entity brackets in '%s'." % (
                            entity[0])
                            warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                 nodelineno=sentence_line + iline)
                        seen1 = True
            ...
            if len(bridge) > 0:
                match = re.match(r'^Bridge=([^(< :>)]+<[^(< :>)]+(:[a-z]+)?(,[^(< :>)]+<[^(< :>)]+(:[a-z]+)?)*)$',
                                 bridge[0])
                if not match:
                    ...
                    for b in bridges:
                        ...
                        if match:
                            ...
                            if not tgteid in starting_mentions:
                                ...
                                testmessage = "Bridge relation '%s' must be annotated at the beginning of a mention of entity '%s'." % (
                                b, tgteid)
                                warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                     nodelineno=sentence_line + iline)
                            ...
                            if bridgekey in entity_bridge_relations:
                                if relation != entity_bridge_relations[bridgekey]['relation']:
                                    ...
                                    testmessage = "Bridge relation '%s' type does not match '%s' specified earlier on line %d." % (
                                    b, entity_bridge_relations[bridgekey]['relation'],
                                    entity_bridge_relations[bridgekey]['line'])
                                    warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                         nodelineno=sentence_line + iline)
                            else:
                                entity_bridge_relations[bridgekey] = {'relation': relation,
                ...
                if not match:
                    ...
                    for a in antecedents:
                        ...
                        if match:
                            ...
                            if srceid == tgteid:
                                ...
                                testmessage = "SplitAnte must not point from an entity to itself: '%s'." % (
                                            srceid + '<' + tgteid)
                                warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                     nodelineno=sentence_line + iline)
                            elif not tgteid in starting_mentions:
                                testid = 'misplaced-splitante-statement'
                                testmessage = "SplitAnte relation '%s' must be annotated at the beginning of a mention of entity '%s'." % (
                                a, tgteid)
                                warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                     nodelineno=sentence_line + iline)
                            if srceid + '<' + tgteid in srctgt:
                                testid = 'repeated-splitante-relation'
                                testmessage = "SplitAnte relation '%s' must not be repeated in '%s'." % (
                    ...
                    for tgteid in tgtante:
                        if len(tgtante[tgteid]) == 1:
                            ...
                            testmessage = "SplitAnte statement '%s' must specify at least two antecedents for entity '%s'." % (
                        ...
                        if tgteid in entity_split_antecedents:
                            if tgtante[tgteid] != entity_split_antecedents[tgteid]['antecedents']:
                                ...
                                testmessage = "Split antecedent of entity '%s' does not match '%s' specified earlier on line %d." % (
                                tgteid, entity_split_antecedents[tgteid]['antecedents'],
                                entity_split_antecedents[tgteid]['line'])
                                warn(testmessage, testclass, testlevel=testlevel, testid=testid,
                                     nodelineno=sentence_line + iline)
                        else:
                            entity_split_antecedents[tgteid] = {'antecedents': str(tgtante[tgteid]),
                                                                'line': sentence_line + iline}
        iline += 1
    if len(open_entity_mentions) > 0:
        testid = 'cross-sentence-mention'
        testmessage = "Entity mentions must not cross sentence boundaries; still open at sentence end: %s." % (
            str(open_entity_mentions))
        warn(testmessage, testclass, testlevel=testlevel, testid=testid, nodelineno=sentence_line + iline)
...
def load_upos_set(filename):
    ...
    """
    res = load_file(os.path.join(PROJECT_ROOT / 'core_utils' / 'tools' / 'ud_validator' / 'data' / filename))
    return res
...
def load_feat_set(filename_langspec, lcode):
    ...
    global featdata
    global warn_on_undoc_feats
    with open(os.path.join(PROJECT_ROOT / 'core_utils' / 'tools'  / 'ud_validator' / 'data' / filename_langspec), 'r', encoding='utf-8') as f:
...
def get_depreldata_for_language(lcode):
    ...
    if lcode == 'ud':
        deprelset = set(
            ['nsubj', 'obj', 'iobj', 'csubj', 'ccomp', 'xcomp', 'obl', 'vocative', 'expl', 'dislocated', 'advcl',
...
def load_set(f_name_ud, f_name_langspec, validate_langspec=False, validate_enhanced=False):
    ...
    """
    res = load_file(os.path.join(PROJECT_ROOT / 'core_utils' / 'tools' / 'ud_validator' / 'data' / f_name_ud))
    # Now res holds UD
    # Next load and optionally check the langspec extensions
    if f_name_langspec is not None and f_name_langspec != f_name_ud:
        path_langspec = os.path.join(PROJECT_ROOT / 'core_utils' / 'tools' / 'ud_validator' / 'data' / f_name_langspec)
...
def get_auxdata_for_language(lcode):
    ...
    if lcode == 'shopen':
        for lcode1 in auxdata.keys():
            ...
            auxlist = auxlist + [x for x in lemmalist if
                                 len([y for y in auxdata[lcode1][x]['functions'] if y['function'] != 'cop.PRON']) > 0]
            coplist = coplist + [x for x in lemmalist if len([y for y in auxdata[lcode1][x]['functions'] if
                                                              re.match("^cop\.", y['function'])]) > 0]
    else:
        lemmalist = auxdata.get(lcode, {}).keys()
        auxlist = [x for x in lemmalist if
                   len([y for y in auxdata[lcode][x]['functions'] if y['function'] != 'cop.PRON']) > 0]
        coplist = [x for x in lemmalist if
                   len([y for y in auxdata[lcode][x]['functions'] if re.match("^cop\.", y['function'])]) > 0]
...
if __name__ == "__main__":
    ...
    io_group.add_argument('--quiet', dest="quiet", action="store_true", default=False,
                          help='Do not print any error messages. Exit with 0 on pass, non-zero on fail.')
    io_group.add_argument('--max-err', action="store", type=int, default=20,
    ...
    list_group.add_argument("--lang", action="store", required=True, default=None,
    ...
    list_group.add_argument("--level", action="store", type=int, default=5, dest="level",
    ...
    tree_group.add_argument("--multiple-roots", action="store_false", default=True, dest="single_root",
    ...
    meta_group.add_argument("--no-tree-text", action="store_false", default=True, dest="check_tree_text",
                            help="Do not test tree text. For internal use only, this test is required and on by default.")
    meta_group.add_argument("--no-space-after", action="store_false", default=True, dest="check_space_after",
    ...
    coref_group.add_argument('--coref', action='store_true', default=False, dest='check_coref',
    ...
    if args.lang:
        ...
        tagsets[TOKENSWSPACE] = [re.compile(regex, re.U) for regex in
                                 tagsets[TOKENSWSPACE]]  # ...turn into compiled regular expressions
        ...
        with open(os.path.join(PROJECT_ROOT / 'core_utils' / 'tools' / 'ud_validator' / 'data' / 'data.json'), 'r', encoding='utf-8') as f:
    ...
    if passed:
        ...
        for f_name in sorted(warn_on_missing_files):
            filepath = os.path.join(PROJECT_ROOT / 'core_utils' / 'tools' / 'ud_validator' / 'data' / f_name + '.' + args.lang)
